...
The relation between EAR and APR (here $r_{cc}$) is:
$$
1 + \text{EAR} = \exp(r_{cc})
$$
so:
...
- The realized rate of return is the *holding period return* (HPR) above
- Recall we calculate HPR as:
$$
\text{HPR} = \frac{\text{Ending price of a share} - \text{Beginning price} + \text{Cash dividend}}{\text{Beginning price}}
$$
...
- The *arithmetic* average is an unbiased estimate of *expected* returns
- The *geometric* average is an HPR that would compound to the same terminal value as the historical data:
$$
