...
- We perform *scenario analysis* to consider the HPRs of various economic scenarios and the probabilities of each scenario
...
E(r) = \sum_s p(s)r(s)
$$
where $p(s)$ and $r(s)$ are the probability and HPR of each scenario $s$
...
$$
- In practice, we use the square root of the variance of returns (i.e., $\sigma = \sqrt{\sigma^2}$), which is the standard deviation of returns and has the same units as returns (i.e., % instead of %\textsuperscript{2})
...
![](./Figures/summary_05.png){height=75%, fig-align="center"}

### Key equations from @bodie_investments_2023 {.appendix}

![](./Figures/key_eq_05.png){height=75%, fig-align="center"}
