...
- The bond issuer (i.e., borrower) agrees to make specified payments to the bondholder on specified dates
- Par value (or face value) is the payment to the bondholder on the bond's maturity date
- Coupon rate is the bond's interest payment per year per dollar of par value
	- Coupon payments are typically split into two equal semiannual payments
	- *Coupon rates are typically set to induce investors to pay par value for a bond upon issuance* 
...
- Denominations are as small as $100, but $1,000 is more common

### Investors do not pay the quoted prices for bonds because of accrued interest {.allowframebreaks}
...
$$
- This practice explains why the price of a maturing bond is par value instead of par value plus one coupon
...
- Call provisions allow issuers to repurchase bonds at a specified *call price* before maturity 
	...
	- Typically come with a period of call protection
	- The option to call a bond is valuable *to the firm*, so callable bonds are typically issued with higher coupon rates and promised yields than non-callable bonds
...
- Put bonds give the holder the option to exchange the bond for par value at some date or to extend for a given number of years
...
- *Foreign bonds* are denominated in the currency of the country where issued
	- Samurai bonds
	- Bulldog bonds
- *Eurobonds* are denominated in a currency *other than* the country where issued, usually the currency of the issuer
	- Euro-yen bonds
	- Euro-sterling bonds
...
- Most bonds have maturities of 30 years or less, however, some have maturities between 50 and 100 years
...
- Catastrophe bonds' final payment depends on whether there has been a specified catastrophe, allowing the issuer to transfer catastrophe risk to the market
...
\text{Accrued interest} = \frac{\text{Ann. coupon}}{2} \times \frac{\text{Days since last coupon}}{\text{Days separating coupons}}
$$
- The sale, or invoice, price of the bond equals the stated price (flat price) plus the accrued interest:
$$
...
- Yield to maturity (YTM)
	...
	- The compound rate of return over the life of the bond, assuming all coupons are reinvested at that YTM
...
### Yield to call (YTC) and YTM are different {.allowframebreaks}
...
### Zero-coupon bonds sell at a discount and pay "interest" as a "capital gain"
...
### Bond rating agencies analyze the levels and trends of issuer financial ratios {.allowframebreaks}
...
### Edward Altman shows that financial ratios predict bankruptcy {.allowframebreaks}
...
- Subordination clause restricts the amount of additional borrowing by the firm
...
- Collateral is a particular asset that the bondholders receive if the firm defaults
	- Collateralized bonds are generally safer than debenture bonds
...
- But, CDS buyers can also use them to speculate on the financial health of an issuer
...
- Mortgage-backed CDOs were an investment disaster from 2007 to 2009
...
![](./Figures/key_eq_14.png){height=75%}