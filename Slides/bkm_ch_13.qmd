...
- The most common tests use a two-pass approach
	1. Use time-series returns to estimate systematic risk ($\beta$)
...
$$
- We can test this relation with a large sample of monthly stock returns for the same stocks over time; for example:
	- $r_{it}$ for 100 stocks over 60 months
	- $r_{Mt}$ for the CRSP market index over 60 months
...
- We collect the following statistics:
	...
	- $b_i$ is the $\beta$ estimate for each of the 100 stocks
...
- We we add $\sigma^2(e_i)$ and estimate:
$$
...
- There  are several difficulties with these two-pass tests
	...
	1. We have fundamental concerns about the structure of the two-pass tests:
		1. The market index used is not the CAPM's "market portfolio"
		1. First-stage betas are estimated with sampling error
		1. Investors cannot borrow at the risk-free rate
...
1. The CAPM is not testable unless *all assets* are included in the sample
1. Using  a market index proxy is subject to two difficulties:
	1. The proxy may be mean-variance efficient even when the true market portfolio is not
	1. Most proxies are correlated, but different proxies can lead to different conclusions
...
### The @fama_common_1993 3-factor model (and its variants) is state-of-the-art {.allowframebreaks}

- @fama_common_1993 add two factors to the CAPM based to explain:
	- Higher book-to-market firms experience higher returns (HML)
	- Smaller firms experience higher returns (SMB)
- They find that value and size are priced risk factors, consistent with the APT

![CAPM versus the Fama and French model. The figure plots the average actual returns versus returns predicted by the CAPM and the FF model for the 25 size and book-to-market double-sorted portfolios. [@bodie_investments_2023, Figure 1]](./Figures/bod12665_1301.png){height=50%}
...
- However, WML is even harder to interpret as an obvious source of risk
...
- Popular measures of liquidity focus on the price impact dimension
...
- Pastor and Stambaugh:
	...
	- Original price change was a concession on the part of trade initiators who needed to offer/accept higher/lower purchase prices to complete their trade promptly
- Amihud:
...
- Sadka
	...
	- Liquidity of firms can vary as the prevalence of informationally motivated trades varies
...
### The *equity premium puzzle* is that returns are too high given that consumption varies so little
 
 - The *equity premium puzzle* is:
...
- The illiquidity premium may the same order of magnitude as the market risk premium
- Part of the average excess return is likely compensation for  *liquidity*  risk rather than just the (systematic) *volatility* of returns
...
![](./Figures/summary_13.png){height=50%}


### Key equations from @bodie_investments_2023 {.appendix}

![](./Figures/key_eq_13.png){height=50%}